...
async function initializeFirebase() {
    try {
        ...
        if (!process.env.FIREBASE_PROJECT_ID || !process.env.FIREBASE_CLIENT_EMAIL || !process.env.FIREBASE_PRIVATE_KEY) {
            console.log('❌ متغيرات Firebase مفقودة من متغيرات البيئة');
            return false;
        }

        // استبدال الأحرف الخاصة في المفتاح الخاص
...
// 🔧 CONFIGURATION - UPDATED FOR DUAL PAYMENT FROM ENV VARIABLES
const CONFIG = {
    ...
    SUBSCRIPTION_PRICES: {
        binance: {
            week: parseFloat(process.env.PRICE_BINANCE_WEEK) || 10,
            month: parseFloat(process.env.PRICE_BINANCE_MONTH) || 30,
            three_months: parseFloat(process.env.PRICE_BINANCE_THREE_MONTHS) || 80,
            year: parseFloat(process.env.PRICE_BINANCE_YEAR) || 250
        },
        bank: {
            week: parseFloat(process.env.PRICE_BANK_WEEK) || 10,
            month: parseFloat(process.env.PRICE_BANK_MONTH) || 30, 
            three_months: parseFloat(process.env.PRICE_BANK_THREE_MONTHS) || 80,
            year: parseFloat(process.env.PRICE_BANK_YEAR) || 250
        }
    },
    ...
    PAYMENT_LINKS: {
        binance: {
            ...
            three_months: process.env.PAYMENT_BINANCE_THREE_MONTHS || "https://binance.com/payment/3months",
        ...
        bank: {
            ...
            three_months: {
                account: process.env.BANK_ACCOUNT_THREE_MONTHS || "1234567890",
                image: process.env.BANK_IMAGE_THREE_MONTHS || "https://i.ibb.co/default-bank-3months.jpg",
                description: process.env.BANK_DESCRIPTION_THREE_MONTHS || "🔹 تحويل بنكي - باقة 3 أشهر\n💳 رقم الحساب: 1234567890\n🏦 البنك: البنك الكريمي\n💰 المبلغ: 80$\n💵 العملة: الدولار الأمريكي\n\n📋 الشروط:\n• يجب التحويل بالدولار الأمريكي\n• إرفاق صورة إثبات الدفع\n• كتابة رقم حساب 1xBet الخاص بك"
            },
            year: {
    ...
    VERSION: process.env.VERSION || "16.0.1",
    DEVELOPER: process.env.DEVELOPER || "♛𝑨𝒎𝒆𝒆𝒏 𝑨𝒍𝒛𝒘𝒂𝒉𝒊♛",
    CHANNEL: process.env.CHANNEL || "@GEMZGOOL",
...
const requiredEnvVars = ['BOT_TOKEN', 'ADMIN_ID', 'CHANNEL_ID', 'CHANNEL_USERNAME'];
...
class EnhancedDatabaseManager {
    ...
    async getUserByOneXBet(onexbet) {
        try {
            ...
            console.error('Error getting user by onexbet:', error);
            return null;
        }
    }
...
async function reconnectAlgorithm(ctx, userData) {
    ...
    try {
        ...
        await ctx.replyWithMarkdown('❌ *حدث خطأ في إعادة ربط الخوارزمية*', getMainKeyboard());
    }
}
...
🔢 *الآن اضغط على "🔐 إدخال رقم الحساب" لبدء التسجيل*
        ...
        await ctx.replyWithMarkdown('❌ حدث خطأ غير متوقع، يرجى المحاولة لاحقاً', getMainKeyboard());
    }
});
