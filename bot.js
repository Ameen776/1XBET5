    ...
    PAYMENT_METHODS: {
        ...
        bank: {
            week: { 
                ...
                image: "https://i.ibb.co/SwF1mfWK/f89eea1a421a.jpg" 
            },
            month: { 
...
async function handleAdminSelectSubscriptionEdit(ctx, text) {
    try {
        ...
        if (paymentMethod === 'binance') {
            ...
            const currentPaymentMethod = settings.payment_methods[paymentMethod][subscriptionType];
            currentValue = `رقم الحساب: ${currentPaymentMethod.account} | 🖼️ صورة البنك الكريمي`;
        }
    ...
    try {
        ...
        if (!isNaN(text) && parseFloat(text) > 0) {
            ...
            await ctx.reply(
                ...
                `💳 ${paymentMethodName}\n\n` +
                `${paymentMethod === 'bank' ? `📝 *الآن أرسل رقم الحساب الجديد:*` : `📝 *الآن أرسل رابط الدفع الجديد أو صورة QR:*`}`
            );
...
• أسبوعي: ${settings.payment_methods.bank.week.account} | 🖼️ صورة البنك الكريمي
• شهري: ${settings.payment_methods.bank.month.account} | 🖼️ صورة البنك الكريمي
• 3 أشهر: ${settings.payment_methods.bank.three_months.account} | 🖼️ صورة البنك الكريمي
• سنوي: ${settings.payment_methods.bank.year.account} | 🖼️ صورة البنك الكريمي
        `;
