...
// 🆕 نظام الجلسات المحسن - إصلاح كامل
const getSessionKey = (ctx) => {
    return ctx.from && ctx.from.id.toString();
};

// 🆕 middleware محسن للجلسات
    ...
    try {
        ...
        if (savedSession) {
            ...
            console.log(`✅ Session restored for user ${sessionKey}:`, {
                ...
                userData: ctx.session.userData ? 'exists' : 'missing',
                verified: ctx.session.step === 'verified'
            });
        } else {
            // جلسة جديدة فقط إذا لم يكن هناك جلسة محفوظة
            ctx.session = {
                step: 'start',
                userData: {},
        ...
        if (ctx.session) {
            ...
            console.log(`💾 Session saved for user ${sessionKey}, step: ${ctx.session.step}`);
        }
    } catch (error) {
        console.error('Session middleware error:', error);
        // في حالة الخطأ، نتابع بدون جلسة
        await next();
    }
});
