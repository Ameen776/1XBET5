...
// 🆕 MIDDLEWARE محسن لتحميل الجلسات
bot.use(async (ctx, next) => {
    ...
    try {
        // 🆕 محاولة تحميل الجلسة من Firestore أولاً
        const savedSession = await dbManager.getSession(sessionKey);
        
        if (savedSession) {
            ctx.session = savedSession;
            console.log(`✅ Session restored for user ${sessionKey}, step: ${ctx.session.step}`);
        } else {
            // جلسة جديدة فقط إذا لم يكن هناك مستخدم مسجل
            const existingUser = await dbManager.getUser(sessionKey);
            if (existingUser) {
                // 🆕 تحميل جلسة من بيانات المستخدم المسجل
                ctx.session = {
                    step: 'verified',
                    userData: existingUser,
                    verificationCode: null,
                    accountId: existingUser.onexbet,
                    paymentType: null,
                    paymentSystem: null,
                    adminMode: false,
                    adminStep: null,
                ...
                console.log(`🔄 Session created from user data for ${sessionKey}`);
            } else {
                // مستخدم جديد تماماً
                ctx.session = {
                    step: 'start',
        ...
        if (ctx.session) {
            ...
            console.log(`💾 Session saved for ${sessionKey}, step: ${ctx.session.step}`);
        }
    } catch (error) {
        console.error('Session middleware error:', error);
        // في حالة الخطأ، استمر بالعملية بدون جلسة
        ctx.session = {
            step: 'start',
            userData: {},
            verificationCode: null,
...
bot.on('text', async (ctx) => {
    try {
        ...
        console.log(`📨 Text from ${userId}: "${text}", session step: ${session.step}`);
...
🔢 *الآن اضغط على "🔐 إدخال رقم الحساب" لبدء التسجيل*
        ...
        // 🔐 STEP 2: Verify Code
            ...
            if (parseInt(text) === ctx.session.verificationCode) {
                ...
                // 🆕 حفظ الجلسة يدوياً بعد التسجيل الناجح
                await dbManager.saveSession(userId, ctx.session);
                console.log(`✅ Manual session save after registration for ${userId}`);
