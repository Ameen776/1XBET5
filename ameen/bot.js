...
// 🆕 ENHANCED: DUPLICATE ACCOUNT PREVENTION + SESSION MANAGEMENT + ANALYSIS TRACKING
// ===================================================
...
const CONFIG = {
    ...
    CHANNEL_USERNAME: process.env.CHANNEL_USERNAME || "@GEMZGOOLBOT", // 🆕 تحديث اسم القناة
    ...
    VERSION: "16.0.0",
    DEVELOPER: "♛𝑨𝒎𝒆𝒆𝒏 𝑨𝒍𝒛𝒘𝒂𝒉𝒊♛",
    CHANNEL: "@GEMZGOOLBOT", // 🆕 تحديث اسم القناة
...
class PersistentStorage {
        ...
        try {
            if (db) {
                const backupData = {
                    ...
                    user_sessions: Array.from(this.userSessions.entries()), // 🆕 نسخ احتياطي للجلسات
                    timestamp: new Date().toISOString(),
                    version: CONFIG.VERSION
                };
    ...
    async saveUserSession(userId, sessionData) {
        try {
            ...
            // 💾 SAVE TO FIREBASE
            if (db) {
                await db.collection('user_sessions').doc(userId.toString()).set(sessionToSave, { merge: true });
            }
    ...
    async getUserSession(userId) {
        try {
            // 🔄 TRY FIREBASE FIRST
            if (db) {
                const sessionDoc = await db.collection('user_sessions').doc(userId.toString()).get();
    ...
    async saveUser(userId, userData) {
        try {
            const completeUserData = {
                ...
                current_bet: userData.current_bet || 0, // 🆕 مبلغ الرهان الحالي
                original_bet: userData.original_bet || 0, // 🆕 مبلغ الرهان الأصلي
                session_profit: userData.session_profit || 0, // 🆕 أرباح الجلسة
                last_updated: new Date().toISOString(),
    ...
    async syncAllDataToFirebase() {
        try {
            ...
            for (const [userId, sessionData] of sessions) {
                await db.collection('user_sessions').doc(userId.toString()).set(sessionData, { merge: true });
            }
    ...
    async backupData() {
        try {
            const backupData = {
                ...
                user_sessions: Array.from(this.storage.userSessions.entries()), // 🆕 إضافة الجلسات
                timestamp: new Date().toISOString()
            };
...
class GoalPredictionAI {
    ...
    getUserAlgorithm(userId) {
        if (!this.userAlgorithms.has(userId)) {
            ...
            this.userAlgorithms.set(userId, {
                ...
                variation: 10 + (seed % 15), // 10-25%
                pattern: seed % 3 // نمط مختلف لكل مستخدم
            });
        }
    ...
    generateSmartPrediction(userId) {
        ...
        const variation = algorithm.variation;
        
        // تعديل الاحتمالية بناءً على الخوارزمية الفريدة
        const adjustedProbability = baseProb + (Math.random() * variation - variation/2);
        const probability = Math.max(40, Math.min(90, Math.floor(adjustedProbability)));
        ...
        const prediction = {
            ...
            timestamp: realTime,
            algorithm: `${this.algorithmVersion}-U${userId.substring(0, 4)}` // 🆕 إضافة معرف المستخدم للخوارزمية
        };
...
// 🛡️ تحقق من الاشتراك قبل كل أمر - محسن مع نظام الكاش (التعديل المطلوب)
bot.use(async (ctx, next) => {
    try {
        ...
        // التحقق من الاشتراك في القناة مع نظام الكاش (10 دقائق)
        const now = new Date();
        const lastCheck = userData.last_subscription_check ? new Date(userData.last_subscription_check) : null;
        const shouldCheck = !lastCheck || (now - lastCheck) > 10 * 60 * 1000; // 10 دقائق

        let isSubscribed = userData.channel_subscribed;

        if (shouldCheck) {
            console.log(`🔄 Performing channel check for user: ${userId}`);
            isSubscribed = await checkChannelSubscription(userId);
        } else {
            console.log(`💾 Using cached subscription status for user: ${userId} - ${isSubscribed}`);
        }
...
bot.use(session({ 
    defaultSession: () => ({ 
        ...
        predictionMessageId: null // 🆕 معرف رسالة التوقع
    })
}));
...
const motivationalMessages = {
    win: [
        ...
        "🚀 *إنجاز ممتاز!* 💫\nمهاراتك في التحليل مذهلة!",
        "💎 *توقع دقيق!* 👑\nأنت تتقن فن التحليل بشكل استثنائي!",
        "⭐ *نتيجة مبهرة!* 🌟\nاستمر في طريق النجاح!",
        "🏆 *فوز مستحق!* 🥇\nدقتك في التوقع تستحق الإشادة!"
    ],
    lose: [
        "💪 *لا تستسلم!* 🔄\nالفشل هو بداية النجاح القادم!",
        "🌧️ *بعد المطر تظهر الشمس* 🌈\nاستمر وسوف تنجح!",
        "🚀 *كل عظيم مر بهذه اللحظات* ⭐\nاستمر في المحاولة!",
        "🔄 *هذه مجرد جولة* 💫\nالجولة القادمة ستكون لك!",
        "🌟 *التحدي يصنع الأبطال* 🏆\nاستمر وسوف تصل!"
    ]
};

// 🆕 الحصول على رسالة تحفيزية عشوائية
function getRandomMotivationalMessage(isWin) {
    const messages = isWin ? motivationalMessages.win : motivationalMessages.lose;
...
bot.start(async (ctx) => {
    try {
        ...
        // 🆕 استعادة جلسة المستخدم من Firebase
        ...
        if (existingUser) {
            ...
            const now = new Date();
            const lastAnalysisTime = existingUser.last_analysis_time ? new Date(existingUser.last_analysis_time) : null;
            const analysisTimeout = 5 * 60 * 1000; // 5 دقائق
            
            if (lastAnalysisTime && (now - lastAnalysisTime) > analysisTimeout) {
                ctx.session.algorithmLinked = false;
                
                await ctx.replyWithMarkdown(
                    `⏰ *انتهت جلسة التحليل*\n\n` +
                    `🔗 لقد مر أكثر من 5 دقائق منذ آخر تحليل.\n` +
                    `🔄 يرجى ربط الخوارزمية من جديد لمتابعة التحليلات.\n\n` +
                    `👤 *رقم حسابك:* \`${existingUser.onexbet}\`\n` +
                    `📍 *دولتك:* ${existingUser.country || 'غير محدد'}`,
                    Markup.inlineKeyboard([
        ...
        // 🔐 STEP 2: Verify Code
            ...
            if (parseInt(text) === ctx.session.verificationCode) {
                ...
                const userData = {
                    ...
                    total_bets: 0,
                    total_profit: 0,
                    last_analysis_time: new Date().toISOString(), // 🆕 وقت بدء الجلسة
                    current_bet: 0,
                    original_bet: 0,
                    session_profit: 0,
                ...
                ctx.session.lastAnalysisTime = new Date(); // 🆕 وقت البدء
    ...
    try {
        ...
        if (callbackData === 'relink_algorithm') {
            ...
            if (userData) {
                ...
                await ctx.answerCbQuery('✅ تم ربط الخوارزمية بنجاح!');
                await ctx.deleteMessage();
                
                // 🆕 إرسال رسالة متحركة للربط
                const linkingMessage = await ctx.replyWithMarkdown(
                    '🔄 *جاري ربط الخوارزمية...*\n\n' +
                    '⚡ جاري إعادة الاتصال...\n' +
                    '🔗 جاري تفعيل الخوارزمية...\n' +
                    '🎯 جاري إعداد النظام...\n\n' +
                ...
                await ctx.deleteMessage(linkingMessage.message_id);
                await ctx.replyWithMarkdown(
                    '✅ *تم ربط الخوارزمية بنجاح!*\n\n' +
                    '🎯 يمكنك الآن استخدام زر "جلب التحليل" مرة أخرى.',
                    getMainKeyboard()
                );
        ...
        if (callbackData.startsWith('win_') || callbackData.startsWith('lose_')) {
            ...
            if (isWin) {
                ...
                userData.session_profit = (userData.session_profit || 0) + profit;
                ctx.session.totalProfit += profit;
                
                await ctx.answerCbQuery(`🎊 مبروك! نجح التوقع وربحت ${profit}$`);
                
                // 🆕 إرسال رسالة تحفيزية للفوز
                const motivationalMessage = getRandomMotivationalMessage(true);
                await ctx.replyWithMarkdown(
                    `🎊 *مبروك! نجح التوقع بنجاح* ✨\n\n` +
                    `${motivationalMessage}\n\n` +
                    `💰 ربحت: ${profit}$\n` +
                    `💵 إجمالي أرباح الجلسة: ${userData.session_profit}$\n` +
                    `🏆 إجمالي أرباحك: ${userData.total_profit}$\n\n` +
                ...
                const motivationalMessage = getRandomMotivationalMessage(false);
...
async function handleGetPrediction(ctx, userData) {
    try {
        ...
        if (!ctx.session.currentBet || ctx.session.currentBet <= 0) {
            ...
            // 🆕 حفظ الجلسة
            await dbManager.saveUserSession(ctx.from.id.toString(), ctx.session);
            return;
        }
        ...
        ctx.session.currentPrediction = prediction; // 🆕 حفظ التوقع الحالي
...
async function handleUserStats(ctx, userData) {
    ...
    await ctx.replyWithMarkdown(
        ...
        `🔥 أرباح الجلسة: ${userData.session_profit || 0}$` +
        subscriptionInfo,
        getMainKeyboard()
    );
}
...
async function handleAdminSearchUser(ctx, query) {
    try {
        ...
        users.slice(0, 10).forEach((user, index) => {
            ...
            message += `   📍 ${user.country || 'غير محدد'} | 📊 ${predictions} توقع | 💰 ${profit}$\n\n`;
        });
...
async function handleAdminUsersList(ctx) {
    try {
        ...
        users.slice(0, 10).forEach((user, index) => {
            ...
            message += `   👤 ${user.user_id} | 🔐 ${user.onexbet} | 📍 ${user.country || 'غير محدد'}\n\n`;
        });
...
async function handleAdminActiveUsers(ctx) {
    try {
        ...
        activeUsers.slice(0, 10).forEach((user, index) => {
            ...
            message += `   📦 ${getSubscriptionDisplayName(user.subscription_type)} | ⏳ ${remainingDays} يوم | 📍 ${user.country || 'غير محدد'}\n\n`;
        });
...
async function handleAdminFreeUsers(ctx) {
    try {
        ...
        freeUsers.slice(0, 10).forEach((user, index) => {
            message += `${index + 1}. ${user.username || 'بدون اسم'}\n`;
            message += `   🆓 محاولات: ${user.free_attempts} | 📍 ${user.country || 'غير محدد'}\n\n`;
        });
        ...
        for (const payment of payments) {
            await ctx.replyWithPhoto(
                payment.screenshot_url,
                {
                    ...
                    `📍 الدولة: ${payment.country || 'غير محدد'}\n` +
...
async function handleAdminAcceptedPayments(ctx) {
    try {
        ...
        acceptedPayments.slice(0, 10).forEach((payment, index) => {
            ...
            message += `   💰 ${payment.amount}$ | 📦 ${getSubscriptionDisplayName(payment.subscription_type)} | 💳 ${payment.payment_system === 'binance' ? 'باينانس' : 'بنكي'} | 📍 ${payment.country || 'غير محدد'}\n\n`;
        });
...
bot.launch().then(() => {
    ...
    console.log('🆕 Enhanced Features: Duplicate Prevention + Session Management + Analysis Tracking');
